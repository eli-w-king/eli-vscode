...
> **For automated testing of the Agents window, this skill IS the correct launch path.** Use `launch.sh --agents` (the repo's **"Run Agent-Aware Agents - OSS"** task wraps exactly this: `npm run transpile-client && ./.agents/skills/launch/scripts/launch.sh --agents`). It clones the **signed-in** dev profile (`~/.vscode-oss-dev`, the same authed profile used by "Run and Compile Agents - OSS" / "Run Dev Agents") into a throwaway copy **and** exposes the CDP/inspector ports for Playwright + agent automation. Do **not** automate against a plain `code.sh --agents` launch (e.g. "Run Dev Agents") — that points at the live source profile and opens no debug ports, so it is neither isolation-safe nor automatable. Always launch the **signed-in** build for end-to-end chat/agent flows; if the launched window looks unauthed, ask the user to sign in rather than continuing against a broken profile.
...
- A VS Code checkout with sources built. Run `npm run compile` once (one-shot) or `npm run watch` for incremental rebuilds. Both build the full client **and** all built-in extensions under `extensions/`. You must build the full product to run successfully, building just the client is not enough.
  - **Do not rely on `npm run transpile-client` alone for visual/UI verification.** The fast esbuild transpile populates `out/` but does **not** run the gulp `copy-codicons` step, so `codicon.ttf` is missing and **every icon renders as an empty box (□)**. `document.fonts.check("16px codicon")` returns `false` in that state. If you took the transpile-only shortcut (e.g. to also run unit tests), run `npm run gulp copy-codicons` (which drops the ttf into `src/vs/base/browser/ui/codicons/codicon/`) and copy it into `out/` at the same relative path before screenshotting — otherwise the UI looks broken and you cannot trust "no X button" claims (the button may just be an invisible glyph). The full `npm run compile` does this automatically.
