...
### 4.4 Shared Input (Multi-Session Layout)

When **more than one** session is visible side-by-side, the per-column chat inputs are replaced by a **single shared input** docked as a flush full-width footer at the bottom of the part, spanning the columns and retargeted to the **active** session. A single visible session is unchanged (it keeps its own inline input).

- **Transcript-only columns.** In shared mode each created-chat column renders its `ChatWidget` **transcript-only** (`IChatWidgetViewOptions.renderInput: false`). The mode is passed through `SessionView.openSession` (via `ISessionViewOptions.sharedInputMode`), which recreates the column's chat view when the mode flips. New-session and new-chat-in-session slots are an **exception** — they always keep their own inline input, so the shared input hides while one of them is active.
- **The docked footer.** `SessionsPart` owns a single `ISharedChatInput` (`SharedChatInputView`, an **input-only** `ChatWidget` via `renderTranscript: false`) created lazily via `IChatViewFactory.createSharedInput()` and hosted in a `.sessions-shared-input-dock` footer (a flush full-width strip at the bottom of the part that blends with the column background and is separated only by a 1px top border — see [media/sessionsPart.css](src/vs/sessions/browser/parts/media/sessionsPart.css)). The footer is an **in-flow flex child** of the content area: the content area is a flex column (`.content { display: flex; flex-direction: column }`), the grid takes `flex: 1 1 auto; min-height: 0`, and the dock takes `flex: 0 0 auto`. This physically constrains the grid element to the space above the dock, so the grid's full-height column separator (sash) can never bleed down through the input. `SessionsPart._layoutContentAndDock` lays out the hosted input, measures the dock, and feeds the grid the remaining inner height. The hosted input is capped to a comfortable composer width (`SessionsPart.SHARED_INPUT_MAX_WIDTH`) and centered by the dock (`align-items: center`) so it does not stretch into a sparse, lopsided bar with the toolbars marooned at the far edges. It also collapses the standalone input part's 32px transcript-alignment inset (`.shared-chat-input-view .interactive-input-part` in [contrib/chat/browser/media/sharedChatInput.css](src/vs/sessions/contrib/chat/browser/media/sharedChatInput.css)) and uses the `inputBackground` token so it reads as a tight, consistent card rather than a colour-mismatched box floating in whitespace. It is bound to the active session's active chat via an autorun in `SessionsPart._updateSharedInput`, shown only when the active session is a created chat.
- **Active vs inactive cue.** Because the columns lose their own inputs, the active column is given an accent bar along its top edge (an inset `focusBorder` box-shadow) and the inactive columns are strongly receded (`opacity: 0.45`). To tie the shared input to the session it targets, a thin `focusBorder` **accent strip** (`.sessions-shared-input-dock__accent`) overlays the dock's top border and is sized/positioned (`SessionsPart._alignSharedInputAccent`) to span only the active column — a quiet connector from the input up to its session. Alignment runs on the next animation frame after layout (`scheduleAtNextAnimationFrame`, so it reads settled bounds) and a `ResizeObserver` on the active column re-aligns it when columns reflow without a part relayout (e.g. the Changes/Files panel collapses). The active/inactive treatments are scoped to a `shared-input-mode` class (toggled alongside `is-active` in `SessionsPart.updateVisibleSessions`) so a lone session column is never accented or dimmed. See `.session-view.shared-input-mode` rules in [media/sessionView.css](src/vs/sessions/browser/parts/media/sessionView.css).
- **Keyboard switching cue.** `Cmd/Ctrl+1..9` (`sessions.focusSessionInGrid{n}`) focuses the session in that grid slot, switching the active session — and `SessionsPart.focusSession` moves keyboard focus straight into the shared input, so the user can keep typing. To make this discoverable, each column's header shows that shortcut as a small native keybinding pill (`SessionHeader.setShortcutHint`, styled with the `keybindingLabel` tokens so it reads as a built-in affordance) only in the shared-input layout. `SessionsPart` resolves the label via `IKeybindingService.lookupKeybinding` and pushes it through `SessionView.setShortcutHint` alongside `is-active`; the active column's pill is fully opaque, inactive ones are quietened until hovered. Living in the header chrome keeps it clear of the scrolling transcript and the input's own toolbar.
- **Accessibility.** The dock is a `role="region"` whose accessible name names the bound session (`localize('sharedInput.regionFor', "Chat input for {0}", name)`), and switching the active session announces the retarget to screen readers via `aria` `status()` (`"Chat input now sends to {0}"`), so non-sighted users get the same "which session am I typing to" cue the accent strip gives sighted users. The shortcut pills are `aria-hidden` (the commands are already in the keybindings/command palette).
...
- **Both react to one model.** A created chat is shown by its column's transcript-only widget and the input-only docked widget at the same time; both observe the same `IChatModel`, so submitting from the footer appears in the active column's transcript.
- **Focus.** `SessionsPart.focusSession` moves keyboard focus into the docked input (not the transcript-only column) when that column is the active created chat.
- **Inactive progress.** Because inactive columns have no input, an inactive session that is `InProgress` shows the looping "comet" border around its **header** (`.session-view.inactive-working` in [media/sessionView.css](src/vs/sessions/browser/parts/media/sessionView.css)) instead of around an input. The active session shows progress on the docked input as usual.
- **Confirmations / "agent needs input".** These render **inline in each session's transcript** (the chat list's fallback when there is no docked input), rather than docked above the input.
