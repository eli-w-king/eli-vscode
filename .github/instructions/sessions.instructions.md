...
- **Run/play-button launches should be shared back to the agent.** If a user starts a task from the Agents window Run/play button and that task opens an interactive surface (for example Code OSS, a browser, a dev server, or a preview URL), treat the launched surface as agent-owned context: attach with Playwright/CDP or the appropriate browser monitoring tool, verify it loaded, observe console/network failures where possible, and report the result back in the transcript. Do not rely on the user to inspect it manually.
